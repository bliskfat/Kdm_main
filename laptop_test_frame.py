...
class InfoBoard(Frame):
    ...
    def auto_scroll(self):
        scroll_speed = 1  # Adjust the scrolling speed here
    ...
    def update_image(self):
        if self.image_paths:
            ...
            image = image.resize((180, 130), Image.LANCZOS)
    ...
    def create_widgets(self):
        ...
        logo = logo.resize((650, 120), Image.LANCZOS)
        ...
        style.configure("Treeview", background="#D3D3D3", foreground="black",
                        rowheight=35, fieldbackground="#D3D3D3")

        style.configure("Treeview.Heading", font=("Arial", 35))  # Set the font size here for the column headings
        # configure the rows in the tree
        style.configure("Treeview", font=("Arial", 30),rowheight=35)  # Set the font size here
        ...
        self.tree.column("fleet_number", anchor=W, width=300)
        self.tree.column("parts_description", anchor=W, width=400)
        self.tree.column("in_stock_since", anchor=CENTER, width=300)
        self.tree.column("days_in_stock", anchor=CENTER, width=300)
        ...
        fg_colour = "black"
        #fg_colour = "white"
        self.tree.tag_configure('warning', background='red',foreground=fg_colour)
        self.tree.tag_configure('acceptable', background='yellow')
        self.tree.tag_configure('good', background='green',foreground=fg_colour)
...
def cycle_frames():
    ...
    root.after(10000, cycle_frames)


# Create the main window
root = Tk()
root.title("KDM Hire - Info Board")
define_size(root, 1400, 750)
...
for division, image_paths in divisions:
    ...
    #frame.auto_scroll()
    auto_scroll(frame.tree, frame.master)
    frames.append(frame)
    frame.pack_forget()
