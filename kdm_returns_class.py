    ...
    for record in records:
        ...
        if count % 2 == 0:
            tree.insert(parent='', index='end', iid=count, text='',
                        values=(record[0], record[3], record[2], record[3], record[4], record[6], record[7]),
                        # values=(record[0], record[3], record[4],record[5]),
                        tags=('evenrow',))
        else:
            tree.insert(parent='', index='end', iid=count, text='',
                        values=(record[0], record[2], record[0], record[4], record[5], record[6], record[7]),
                        # values=(record[0], record[3], record[4], record[5]),
                        tags=('oddrow',))
        # increment counter
        count += 1
...
class GoodsReturnClass:
    ...
    def add_new_record_window(self):
        ...
        worker_options = ["Worker 1", "Worker 2", "Worker 3", "Worker 4"]
        worker_label = Label(self.add_new_record_frame, text='Select Worker', font=("Arial", 30))
        worker_label.grid(row=7, column=0, padx=10, pady=10)

        self.worker_combobox = ttk.Combobox(self.add_new_record_frame, values=worker_options, font=("Arial", 30))
        self.worker_combobox.grid(row=7, column=1, padx=10, pady=10)
        self.worker_combobox.set(worker_options[0])  # Set the default selected worker
    ...
    def search_by_fleet_number(self):
        ...
        cursor = connection.cursor()
        with connection:
            cursor.execute("SELECT rowid, * FROM returns WHERE fleet_number like ?", (lookup_record,))
            records = cursor.fetchall()
    ...
    def update_details(self):
        ...
        with sqlite3.connect(database) as connection:
            cursor = connection.cursor()
            cursor.execute("""UPDATE returns SET fleet_number = ?,parts_description = ?,in_stock_since = ?
                WHERE oid = ?
            """, (flee_number, description, date, oid))

        self.clear_entry_boxes()
        messagebox.showinfo("Vor Returns Update", "Record Updated.")
